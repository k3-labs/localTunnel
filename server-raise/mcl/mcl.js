...
/* Adapted from https://github.com/thehubbleproject/hubble-contracts/blob/f1c13fe4e1a0dc9ab1f150895de7c0e654ee46b0/ts/
   ...
   export function getSigningKey(seed) {
       return parseFr(prefixSeed(seed));
   }
   
   export function getPublicKey(signingKey) {
       const pubKey = mcl.mul(g2(), signingKey);
       pubKey.normalize();
       return pubKey; 
   }
   
   export function sign(signingKey, message) {
     return _sign(signingKey, message, taskManagerDomain());
   }
   
   export function signRegistrationMessage(signingKey, message) {
     return _sign(signingKey, message, operatorRegistrationDomain());
   }
   
   export function signVerificationMessage(signingKey, message) {
     return _sign(signingKey, message, operatorVerificationDomain());
   }
   
   function _sign(signingKey, message, domain) {
   ...
   export function hashToPoint(message, domain) {
   ...
   function taskManagerDomain() {
       return ethers.getBytes(
           ethers.solidityPackedKeccak256(['string'], ['TasksManager'])
       );
   }
   
   export function parseG1(solG1) {
     const g1 = new mcl.G1();
     const [x, y] = solG1;
     g1.setStr(`1 ${x} ${y}`, 16);
     return g1;
   ...
   function operatorRegistrationDomain() {
     return ethers.getBytes(
         ethers.solidityPackedKeccak256(['string'], ['K3_REGISTRATION'])
     );
   }
   
   function operatorVerificationDomain() {
     return ethers.getBytes(
         ethers.solidityPackedKeccak256(['string'], ['K3_VERIFICATION'])
     );
   }

   export function operatorLocalTunnelDomain() {
    return ethers.getBytes(
        ethers.solidityPackedKeccak256(['string'], ['K3_LOCAL_TUNNEL'])
    );
  }